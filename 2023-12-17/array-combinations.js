...
const arrayCombinations = (arr) => {
    // how many unique elements in each array? multiply by each other
    return arr
        .map((value) => new Set(value).size)
...
// refactoring it. we don't need to map over the array first
...
const arrayCombinations3 = (arr) => {
    // how many unique elements in each array? multiply by each other
    let result = 1;
    for (const member of arr) {
        result *= new Set(member).size;
    }
    return result;
};

console.log(arrayCombinations3([[1, 2], [4], [5, 6]]), 4);
console.log(
    arrayCombinations3([
        [1, 2],
        [4, 4],
