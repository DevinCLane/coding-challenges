/* 
Majority Element

Given an array nums of size n, return the majority element.
The majority element is the element that appears more than ⌊n / 2⌋ times. You may assume that the majority element always exists in the array.
 
Example 1:
Input: nums = [3,2,3]
Output: 3
Example 2:
Input: nums = [2,2,1,1,1,2,2]
Output: 2

https://leetcode.com/problems/majority-element/
*/

// this returns the most frequently occurring element. Which is similar but not exactly the same as the majority element (or perhaps it's good enough)
const majorityElement = nums => {
    // hash map
    // look for the majority element
    const hashMap = {}
    for (const num of nums) {
        hashMap[num] = ++hashMap[num] || 1;
    }
    let maxNum = null;
    let numCount = 0
    for (const num in hashMap) {
        if (num > maxNum) {
            maxNum = num
            numCount = hashMap[num]
        }
    }
    return +maxNum
}

console.log(majorityElement([3,2,3]), 3)
console.log(majorityElement([2,2,1,1,1,2,2]), 2)

// 
const majorityElement2 = nums => {
    const hashMap = {}
    let count = 0,
        majorityElement = nums[0]

    for (const num of nums) {
        hashMap[num] = hashMap[num] + 1 || 1
    }
    for (const num in hashMap) {
        if (hashMap[num] > count) {
            majorityElement = num;
            count = hashMap[num]
        }
    }
    return +majorityElement
        
}

console.log(majorityElement2([3,2,3]), 3)
console.log(majorityElement2([2,2,1,1,1,2,2]), 2)