/* 
Array Diff

Your goal in this kata is to implement a difference function, which subtracts one list from another and returns the result.

It should remove all values from list a, which are present in list b keeping their order.

arrayDiff([1,2],[1]) == [2]

If a value is present in b, all of its occurrences must be removed from the other:

arrayDiff([1,2,2,2,3],[2]) == [1,3]


arrayDiff([], [4,5]), [], "a was [], b was [4,5]"

arrayDiff([3,4], [3]), [4], "a was [3,4], b was [3]"

arrayDiff([1,8,2], []), [1,8,2], "a was [1,8,2], b was []"

https://www.codewars.com/kata/523f5d21c841566fde000009
*/

const arrayDiff = (a, b) => {
    // probably will want to use map to create a new array that meets our conditions
    // ask each item of array a, 'hello b, pass through this item from a if you do not have it'
    const bSet = new Set(b)
    return a.filter(value => !bSet.has(value))
}

console.log(arrayDiff([1,2,2,2,3],[2]), [1,3])
console.log(arrayDiff([], [4,5]), [], "a was [], b was [4,5]")
console.log(arrayDiff([3,4], [3]), [4], "a was [3,4], b was [3]")
console.log(arrayDiff([1,8,2], []), [1,8,2], "a was [1,8,2], b was []")